...
ball_1 = box(
    ...
    size=vector(side_length, side_length, side_length),
    color=color.purple,
)
ball_2 = box(
    pos=vector(center_cubes_distance, 0, 0),
    velocity=vector(0, 0, 0),
    size=vector(side_length, side_length, side_length),
    color=color.purple,
)
wall = box(
    pos=vector((center_cubes_distance * 2) + 0.1, 0, 0),
    size=vector(0.2, 10, 10),
    color=color.cyan,
)
...
while True:
    ...
    if ball_1.pos.x >= (ball_2.pos.x - side_length):
        ...
        ball_1.velocity.x = (
            ((left_cube_mass - 1) / (left_cube_mass + 1)) * ball_1.velocity.x
        ) + (2 * ball_2.velocity.x) / (left_cube_mass + 1)
        ball_2.velocity.x = ((2 * left_cube_mass / (left_cube_mass + 1)) * temp) + (
            (1 - left_cube_mass) * ball_2.velocity.x
        ) / (left_cube_mass + 1)
    ...
    if (
        ...
        print(
            "calculated pi number for chosen decimal_accuracy (",
            decimal_accuracy,
            ") is: ",
            calculated_pi_number,
        )
